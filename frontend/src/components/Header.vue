<template>
  <header>
    <h1><router-link :to="{ name: 'posts' }">ブログ</router-link></h1>
    <div id="form">
      <input
        type="text"
        placeholder="Search"
      ...
      <div class="selectWrap">
        <select v-model="selected" @change="search" class="select">
...
import { UPDATE_POSTS, UPDATE_CATEGORIES } from "@/store/mutation-types";
...
export default {
  ...
  methods: {
    ...
    search() {
      this.$router.push({
        name: "posts",
        query: { page: 1, keyword: this.keyword, category: this.selected },
      });
    },
  },
};
